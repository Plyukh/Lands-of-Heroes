...
public class ProjectileController : MonoBehaviour
{
    ...
    {
        // 1) Спавним опциональный эффект попадания (до callback'ов или после — по желанию)
        if (impactEffectPrefab != null)
            Instantiate(impactEffectPrefab, transform.position, Quaternion.identity);

        // 2) Первый callback: эффект у цели (например, PlayImpact)
        try { onHitCallback?.Invoke(); }
        catch (Exception ex) { Debug.LogException(ex); }

        // 3) Второй callback: завершающий хук (например, уведомить аниматор атакующего -> он вызовет OnAttackHit)
